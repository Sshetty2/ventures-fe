	...
	if (events.length) {
		return (
			<div className='Filters'>
				...
				<select name='event_type' value={eventType} onChange={e => updateType(e.target.value)}>
					<option key='Type' value=''>
						All Event Types
					</option>
					{setFilters("event_type", events)}
				</select>
				<select name='month' value={month} onChange={e => updateMonth(e.target.value)}>
					<option key='Month' value=''>
						All Months
					</option>
					{setMonthFilter(month, events)}
				</select>
				<select name='state' value={state} onChange={e => updateState(e.target.value)}>
					<option key='State' value=''>
						All States
					</option>
					{setFilters("state", events)}
				</select>
				<button data-tip data-for='reset' className='reset-filters' onClick={() => clearFilters()} />
				<ReactTooltip id='reset' type='dark' effect='solid' place='top'>
					Reset Filters
				</ReactTooltip>
			</div>
		);
